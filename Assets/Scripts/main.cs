...
public class main : MonoBehaviour {
	...
	#region Basic instancing properties
        ...
        [SerializeField] Mesh _template2;
        [SerializeField] Mesh _template3;
        ...
        public Mesh template2 {
            ...
            set { _template2 = value; ReallocateBuffer(); }
        }
        public Mesh template3 {
            get { return _template3; }
            set { _template3 = value; ReallocateBuffer(); }
        }
    #endregion    
	...
	void Start () {
        ...
        cube.transform.localScale= new Vector3(boxSize.x*2.0f,boxSize.y*0.5f,boxSize.z*2.0f);
        ...
        _drawArgsBuffer.SetData(new uint[5] {_template.GetIndexCount(0), (uint)InstanceCount/2, 0, 0, 0});

        _drawArgsBuffer2 = new ComputeBuffer(1, 5 * sizeof(uint), ComputeBufferType.IndirectArguments);
        _drawArgsBuffer2.SetData(new uint[5] {_template2.GetIndexCount(0), (uint)InstanceCount/2, 0, 0, (uint)(InstanceCount/2)});

        _drawArgsBuffer3 = new ComputeBuffer(1, 5 * sizeof(uint), ComputeBufferType.IndirectArguments);
        _drawArgsBuffer3.SetData(new uint[5] {_template3.GetIndexCount(0), (uint)maxParticles, 0, 0, 0});
	...
	void Update () {
        ...
        _compute.SetFloats("gridTextureResolution", new float[]{gridBufferW, gridBufferH});
        ...
        if(!renderParticles) {
            ...
            Graphics.DrawMeshInstancedIndirect(
                _template2, 0, _material, TransformedBounds,
                _drawArgsBuffer2, 0, _props2
            );
        } else {
            Graphics.DrawMeshInstancedIndirect(
            _template3, 0, _material2, TransformedBounds,
    ...
    void updateBroadPhase(){
        int gridPotZ = 1;
        while(gridPotZ * gridPotZ < broadphaseResolution.y) gridPotZ *= 2;
        broadphaseGridZTiling.Set(gridPotZ,gridPotZ);
    }
