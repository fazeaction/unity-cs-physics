Shader "Custom/Particles" {
	...
	SubShader {
		...
		void setup()
        {
            ...
            float3 v1 = applyQuat(float3(radius.x,0.0,0.0), _QuatBuffer[ps.w]);
            float3 v2 = applyQuat(float3(0.0,radius.y,0.0), _QuatBuffer[ps.w]);
            float3 v3 = applyQuat(float3(0.0,0.0,radius.z), _QuatBuffer[ps.w]);
